
<?php $this->load->view('header'); ?>

    <div class="row">
        <p style="margin-left: 1.5%;"><b> Veja quem já contratou nossos serviços </b></p>
        <div class="col-md-5">
            <p> Nome da Empresa 1 </p>
            <img width="375px" height="200px" src="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;"> 
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação. A vantagem de usar Lorem Ipsum é que ele tem uma 
                distribuição.
            </article>
        </div>

        <div class="col-md-5">
            <p> Nome da Empresa 2 </p>
			<img width="375px" height="200px" src="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;">
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação. A vantagem de usar Lorem Ipsum é que ele tem uma 
                distribuição.
            </article>
        </div>
    </div>

    <div class="row" style="margin-top: 2%;">
        <div class="col-md-3">
            <p> Nome da Empresa 3 </p>
            <img width="210px" height="100px" src="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;"> 
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação.
            </article>
        </div>

        <div class="col-md-3">
            <p> Nome da Empresa 4 </p>
            <img width="210px" height="100px" src="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;"> 
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação.
            </article>
        </div>

        <div class="col-md-3">
            <p> Nome da Empresa 5 </p>
            <img width="210px" height="100px" src="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;"> 
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação.
            </article>
        </div>
    </div>

    <div class="row" style="margin-top: 2%;">
        <div class="col-md-3">
            <p> Nome da Empresa 6 </p>
            <img width="210px" height="100px" src="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;"> 
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação.
            </article>
        </div>

        <div class="col-md-3">
            <p> Nome da Empresa 7 </p>
            <img width="210px" height="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;"> 
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação.
            </article>
        </div>

        <div class="col-md-3">
            <p> Nome da Empresa 8 </p>
            <img width="210px" height="100px" src="<?php echo base_url('application/assets/Imgs/CaixaFerramentas.jpg')?>">
            <article style="text-align: justify;"> 
                É um fato conhecido de todos que um leitor se distrairá com 
                o conteúdo de texto legível de uma página quando estiver examinando 
                sua diagramação.
            </article>
        </div>
    </div>

    <div class="row">
        <div class="col-md-5" style="margin-top: 3%;">
            <h5> <b> Opinião de quem contratou </b> </h5>
            <div class="row">
                <div class="col-md-12" style="margin-top: 2%;">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                </div>
            </div>
            <div class="row">
                <div class="col-md-12" style="margin-top: 2%;">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                </div>
            </div>
            <div class="row">
                <div class="col-md-12" style="margin-top: 2%;">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                </div>
            </div>
        </div>
        <?php $this->load->view('noticias'); ?>
    </div>
    
<?php $this->load->view('footer'); ?>
